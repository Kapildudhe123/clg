...
import in.com.college.model.MarksheetModelHibImp;
...
public class MarksheetCtl extends BaseCtl {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		MarksheetModelHibImp model = new MarksheetModelHibImp();
		if (id > 0) {
			MarksheetDTO dto;
			try {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (OP_UPDATE.equalsIgnoreCase(op)) {
			String studentId = DataUtility.getString(request.getParameter("studentId"));
			long id = DataUtility.getLong(request.getParameter("id"));
		...
		if (OP_SAVE.equalsIgnoreCase(op)) {
			...
			MarksheetDTO dto1 = (MarksheetDTO) populateDTO(request);
			try {
				model.delete(dto1);
