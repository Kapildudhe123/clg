...
public class StudentModelHibImp {

	public long add(StudentDTO dto) throws ApplicationException, DuplicateRecordException {
		// TODO Auto-generated method stub
		System.out.println("in add methoddddddddddddddddddddddddddddd");
		Session session = HibDataSource.getSession();
		Transaction tx = null;

		System.out.println("before exitDTO >>= " + dto.getEmailId());

		StudentDTO existDTO = findByEmailId(dto.getEmailId());

		if (existDTO != null && existDTO.getEmailId().length() > 0) {
			throw new DuplicateRecordException("login id alread exists");
		}
	...
	public List search(StudentDTO dto, int pageNo, int pageSize) throws ApplicationException {
		...
		try {
			...
			if (dto != null) {
				...
				/*
				 * if (dto.getCollegeId() >0 ) { criteria.add(Restrictions.eq("collegeId",
				 * dto.getCollegeId())); }
				 */
